...
<body>
    ...
    <?php
    if (isset($_POST['retailername']) || isset($_SESSION['retailername'])) :
        $retailername = $_SESSION['retailername'];
        $query = "SELECT * FROM product where retailername='$retailername'  order by createddate desc";
        $result = mysqli_query($db, $query);
        $fetch = mysqli_fetch_all($result, MYSQLI_ASSOC);
    ?>

        <div>
            <h1>All the below products are provided by <?= $retailername ?> </h1>
        </div>
